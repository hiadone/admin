...
$admin_name="";
foreach(element('boardadminlist', element('list', $view)) as $value){
    $admin_name[]=$value['mem_nickname'];
}
 ?>

<div class="board">

    <?php if(!empty($admin_name)) echo '<h4 class="highlight mb20">게시판 관리자는 '.implode($admin_name,","). '님 입니다.</h4>'; ?>
    <h3><strong>[<?php echo html_escape(element('bgr_name', element('list', $view))); ?>]</strong><?php echo html_escape(element('board_name', element('board', element('list', $view)))); ?></h3>
    <div class="row mb20">
                    ...
                    <td>
                        ...
                        if(element('pln_url',$result))
                            foreach(element('pln_url',$result) as $key=> $value){
                                echo '<div><i class="fa fa-link"></i><a href="'.element('pln_url',$value).'" target="_blank">'.element('pln_url',$value).'</a> <span class="badge" style="font-size:11px">'.element('pln_start',$value).'~'.element('pln_end',$value).'</span></div>';
                            }
                         ?>
                        
                        
                    </td>
                    <td><i class="fa fa-link"></i><a href="<?php echo element('press_url', element('extravars', $result)); ?>" target="_blank"><?php echo element('press_url', element('extravars', $result));?></a></td>
                    <td>
                    <?php 
                    echo element('view_type', element('extravars', $result));
                    ?>
                    </td>
                    <td><a href="javascript:post_action('post_extra', '<?php echo element('post_id', $result);?>', '<?php echo element('popstate', element('extravars', $result)) ==='enable' ? 'disable':'enable';?>');" class="btn <?php echo element('popstate', element('extravars', $result)) ==='enable' ? 'btn-success':'btn-danger';?> btn-xs"><?php echo element('popstate', element('extravars', $result));?></a></td>
                </tr>
            <?php
                }
            }
