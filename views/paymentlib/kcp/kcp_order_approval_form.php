...
?>
    ...
    <form name="sm_form" method="POST" accept-charset="euc-kr">
        ...
        <!-- 사이트 이름 -->
        <input type="hidden" name="shop_name" value="<?php echo $this->cbconfig->item('company_name'); ?>" />
        <!-- 결제수단-->
        <input type="hidden" name="pay_method" value="<?php echo element('pay_method', element('data', $view)); ?>" />
        <!-- 주문번호 -->
        ...
        <input type="hidden" name="Ret_URL" value="<?php echo site_url('payment/kcp_order_approval_form'); ?>" />
        <!-- 인증시 필요한 파라미터(변경불가)-->
