...
class Payment extends CB_Controller
{
    ...
    public function inicis_approval($ptype = '')
    {
        ...
        $view['view']['data'] = $data = unserialize($row['dt_data']);



        if ($ptype === 'deposit') {
            $view['view']['order_action_url'] = site_url('deposit/update');
            $view['view']['page_return_url'] = site_url('deposit');
        } else {
            $view['view']['order_action_url'] = site_url('cmall/orderupdate');
        ...
        if ($this->input->post_get('P_STATUS') !== '00') {
            ...
            $post_data = array(
                'P_MID' => $this->cbconfig->item('pg_inicis_mid'),
         ...
         */
        $this->data = $view;
        $this->layout = '_layout/basic/layout_popup';
    ...
    public function orderdatasave()
    {
        ...
        $limit_time = cdate('Y-m-d H:i:s', (ctimestamp() - 86400 * 1));
        $where = array(
    ...
    public function kcp_order_approval_form($ptype = '')
    {
        ...
        $data['settle_method'] = $settle_method = $this->input->post('settle_method', null, ''); // 결제방법
        ...
        if ($ptype === 'deposit') {
            ...
            $data['order_action_url'] = site_url('deposit/update');
        } else {
            $data['js_return_url'] = site_url('cmall/');
            $data['order_action_url'] = site_url('cmall/orderupdate');
        }
        ...
        $view['view']['pod_data'] = $pod_data = unserialize($row['dt_data']);
    ...
    public function xpay_approval($ptype = '')
    {
        ...
        $LGD_RETURNURL = site_url('payment/lg_returnurl/' . $ptype);
        ...
        $LGD_MTRANSFERWAPURL = site_url('payment/lg_returnurl/' . $ptype);
        ...
        $view['view']['payReqMap'] = $_SESSION['PAYREQ_MAP'] = $payReqMap;
