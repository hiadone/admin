...
class Group extends CB_Controller
{
     ...
     * 모델을 로딩합니다
     */
    protected $models = array('Board','Board_admin');
    ...
    public function index($bgr_key = '')
    {
        ...
        $board_list = array();
        $group_list = array();
        $boardadminlist = array();
        if ($board_id && is_array($board_id)) {
            foreach ($board_id as $key => $val) {
                $board_list[] = $this->board->item_all(element('brd_id', $val));
                $group_list[element('brd_id', $val)] = element('bgr_name', $group);
                $boardadminlist[element('brd_id', $val)] = $this->Board_admin_model
                    ->get_board_admin_member(element('brd_id', $val));
            }
        }

        $view['view']['board_list'] = $board_list;
        $view['view']['group_list'] = $group_list;
        $view['view']['boardadminlist']  =  $boardadminlist;
