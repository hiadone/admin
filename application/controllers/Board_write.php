...
class Board_write extends CB_Controller
{
    ...
    public function _write_common($board, $origin = '', $reply = '')
    {
        ...
        if ($form_validation === false OR $file_error) {
            ...
            $pln_start = $this->input->post('pln_start_post_link');
            $pln_end = $this->input->post('pln_end_post_link');
                ...
                foreach ($post_link as $pkey => $pval) {
                    if ($pval) {
                        $linkupdate = array(
                            ...
                            'pln_start' => $pln_start[$pkey],
                            'pln_end' => $pln_end[$pkey],
                        );
    ...
    public function modify($post_id = 0)
    {
        ...
        if ($form_validation === false OR $file_error) {
            ...
            $pln_start_update = $this->input->post('pln_start_post_link_update');
            $pln_end_update = $this->input->post('pln_end_post_link_update');
            $link_count = 0;
            if ($post_link_update && is_array($post_link_update) && count($post_link_update) > 0) {
                foreach ($post_link_update as $pkey => $pval) {
                    if ($pval) {
                        $linkupdate = array(
                            'pln_url' => prep_url($pval),
                            'pln_start' => $pln_start_update[$pkey],
                            'pln_end' => $pln_end_update[$pkey],
                        );
