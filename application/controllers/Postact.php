...
class Postact extends CB_Controller
{
     ...
     * 게시물 후팝업 활성 및 비활성 하기
     */
    public function post_extra($post_id = 0, $flag = 'disable')
    {
     ...
     * 링크 클릭 하기
     */
    public function popstateStat($post_id = 0,$session_id = '',$link_id = 0)
    {


        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_postactStat';
        $this->load->event($eventname);
        ...
        if(empty($session_id)){
            log_message('error', 'session_id 가 비어 있습니다.');
        }
        ...
        if (empty($link_id) OR $link_id < 1) {
            ...
            preg_match("/^http[s]*:\/\/([\.\-\_0-9a-zA-Z]*)\//", $this->input->get('referer'), $match);
            ...
            $insertdata = array(
                ...
                'post_id' => element('post_id', $post),
                'brd_id' => element('brd_id', $post),
                'pl_datetime' => cdate('Y-m-d H:i:s'),
                'pl_ip' => $this->input->ip_address(),
                'pl_referer' => $this->input->get('referer',null,''),
                'pl_useragent' => $this->agent->agent_string(),
                'pl_session_id' => $session_id,
                'pl_domain' => $s,
            );
            $this->load->model('Popstate_list_model');
            $this->Popstate_list_model->insert($insertdata);
            ...
            $insertdata = array(
                ...
                'post_id' => element('post_id', $post),
                'brd_id' => element('brd_id', $post),
                'pc_datetime' => cdate('Y-m-d H:i:s'),
                'pc_ip' => $this->input->ip_address(),
                'pc_referer' => $this->input->get('referer',null,''),
                'pc_useragent' => $this->agent->agent_string(),
                'pc_session_id' => $session_id,
                'pc_domain' => $s,
            );
            $this->load->model('Popstate_click_list_model');
            $this->Popstate_click_list_model->insert($insertdata);
    ...
    public function getPopstateStat($datetype=null,$ps_datetime = null,$brd_id = null)
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_postact_getPopstateStat';
        $this->load->event($eventname);
        ...
        if (empty($ps_datetime) || empty($datetype)) {
            $result = array('error' => '잘못된 접근입니다.');
            exit(json_encode($result));
        }

        $this->load->model(array('popstate_stat', 'Board'));

        $select = 'ps_id, pln_id, ps_datetime';

        $result = $this->Popstate_stat_model->get_link_click_count($datetype, $ps_datetime, $ps_datetime,$brd_id);
