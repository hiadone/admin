...
class Paymentlib extends CI_Controller
{
    ...
    public function kcp_init()
    {
        ...
        // KCP SITE KEY 입력 체크
        if ($this->CI->cbconfig->item('use_payment_card') OR $this->CI->cbconfig->item('use_payment_realtime') OR $this->CI->cbconfig->item('use_payment_vbank') OR $this->CI->cbconfig->item('use_payment_phone')) {
        ...
        $data['pg_conf_js_url'] = 'https://pay.kcp.co.kr/plugin/payplus' . $data['is_test'] . '_un.js';
    ...
    public function inicis_init($type = '')
    {
        ...
        if ($type !== 'nolib') {
             ...
             * 1. 라이브러리 인클루드 *
             **************************/
            include('plugin/pg/inicis/libs/INILib.php');
        ...
        // 플러그인 호출 URL
        if (is_https() === true) {
            $data['ini_js_url'] = 'https://plugin.inicis.com/pay61_secunissl_crossl.js';
        } else {
            $data['ini_js_url'] = 'http://plugin.inicis.com/pay61_secuni_cross.js';
        }
